...
/// Historical connection metadata
#[derive(Debug, Clone)]
...
impl ConnectionRegistry {
        ...
        if let Some(agent) = connection_lock.take() {
            if agent.connection_id == connection_id {
                ...
                let metadata = AgentMetadata {
                    ...
                    connected_at: chrono::Utc::now().timestamp() - agent.connected_at.elapsed().as_secs() as i64,
    ...
    pub async fn get_stats(&self) -> RegistryStats {
        ...
        RegistryStats {
            ...
            total_commands_executed: current.as_ref().map(|c| c.commands_executed).unwrap_or(0),
        }
    }
...
mod tests {
    ...
    async fn test_single_agent_enforcement() {
        ...
        let result1 = registry.register_agent(&agent1, "conn-1".to_string(), "192.168.1.100".to_string()).await;
        ...
        let result2 = registry.register_agent(&agent2, "conn-2".to_string(), "192.168.1.101".to_string()).await;
    ...
    async fn test_unregister_and_history() {
        ...
        registry.register_agent(&agent, "conn-1".to_string(), "192.168.1.100".to_string()).await.unwrap();
        assert!(registry.is_agent_connected().await);
        
        registry.unregister_agent("conn-1", Some("test disconnect".to_string())).await;
        assert!(!registry.is_agent_connected().await);
